...
const { validateAND, validateOR } = require('../models/alarm');
...
// Add status code to http response in the future
exports.handleAddAlarm = (req, res) => {
    ...
    if (isValidParams && isValidAlarm) {
        ...
        db.addAlarm(alarm).then(ret => {
            res.send({ status: true, id: ret._id });
        });
...
exports.handleGetAlarm = (req, res) => {
    ...
    if (error === undefined) {
        db.getAlarm(req.body).then(ret => res.send({ status: true, data: ret }));
    } else {
        res.send({ status: false, msg: error });
    }
}
...
exports.handleUpdateAlarm = (req, res) => {
    ...
    if (isValidObjectId && isValidParams) {
        ...
        db.updateAlarm(objectId, req.body).then(ret => {
